...
export function Login() {
  ...
  async function handleLogin(e: React.FormEvent) {
    ...
    try {
      ...
      // 1. Desestrutura a resposta do novo DTO (LoginResponseDto)
      const { token, role, mustChangePassword } = response.data;

      // 2. Salva o Token
      localStorage.setItem('token', token);

      // 3. Salva os metadados do usuário (Importante para a Sidebar e PrivateLayout)
      localStorage.setItem('user_data', JSON.stringify({ 
          role, 
          mustChangePassword 
      }));

      // 4. DECISÃO DE ROTA (O Pulo do Gato 🐱)
      if (mustChangePassword) {
          // Se for primeiro acesso, joga para a tela de troca de senha
          navigate('/change-password');
      } else {
          // Se for acesso normal, joga para a Control Tower
    ...
    <>
      ...
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-gray-50 via-gray-100 to-gray-200 relative overflow-hidden">
        ...
        <div className="absolute inset-0 opacity-5">
          <div className="absolute inset-0" style={{ backgroundImage: `repeating-linear-gradient(45deg, transparent, transparent 50px, rgba(0,0,0,0.1) 50px, rgba(0,0,0,0.1) 52px)` }}></div>
        </div>
        ...
        <div className="bg-white p-10 rounded-2xl shadow-2xl w-full max-w-md relative z-20 border border-gray-200 animate-fade-in-up backdrop-blur-sm">
          ...
          <div className="text-center mb-8 relative">
            <div className="inline-block relative group">
              ...
              <img src={dhlLogo} alt="DHL Logo" className="h-14 mx-auto mb-4 object-contain relative z-10 transform group-hover:scale-105 transition-transform duration-300" />
            </div>
            <div className="relative">
              <p className="text-gray-600 font-bold uppercase tracking-[0.3em] text-xs relative z-10">Labor Control System</p>
          ...
          <form onSubmit={handleLogin} className="space-y-6">
            <div className="group relative">
              <label className="block text-sm font-bold text-gray-700 mb-2 transition-colors group-focus-within:text-dhl-red">Usuário</label>
              <div className="relative">
                <div className="absolute left-0 top-0 h-full w-12 flex items-center justify-center text-gray-400 group-focus-within:text-dhl-yellow transition-colors z-10"><User size={20} /></div>
                <input type="text" className="w-full pl-12 pr-4 py-3.5 border-2 border-gray-200 rounded-xl focus:outline-none focus:border-dhl-yellow focus:ring-4 focus:ring-dhl-yellow/10 bg-gray-50 focus:bg-white transition-all duration-300 hover:border-gray-300" placeholder="Ex: thiago_admin" value={username} onChange={(e) => setUsername(e.target.value)} required />
            ...
            <div className="group relative">
              <label className="block text-sm font-bold text-gray-700 mb-2 transition-colors group-focus-within:text-dhl-red">Senha</label>
              <div className="relative">
                <div className="absolute left-0 top-0 h-full w-12 flex items-center justify-center text-gray-400 group-focus-within:text-dhl-yellow transition-colors z-10"><Lock size={20} /></div>
                <input type="password" className="w-full pl-12 pr-4 py-3.5 border-2 border-gray-200 rounded-xl focus:outline-none focus:border-dhl-yellow focus:ring-4 focus:ring-dhl-yellow/10 bg-gray-50 focus:bg-white transition-all duration-300 hover:border-gray-300" placeholder="••••••••" value={password} onChange={(e) => setPassword(e.target.value)} required />
                <div className="absolute bottom-0 left-0 w-0 h-0.5 bg-gradient-to-r from-dhl-yellow to-dhl-red group-focus-within:w-full transition-all duration-500"></div>
              </div>
            </div>
