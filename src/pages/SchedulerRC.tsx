...
import { 
  ...
  Info, Zap, Activity, BrainCircuit, Truck
} from 'lucide-react';
...
export function SchedulerRC() {
  ...
  useEffect(() => {
    const cachedData = localStorage.getItem(CACHE_KEY_RC);
    if (cachedData) setWeekData(JSON.parse(cachedData));
  }, []);

  useEffect(() => {
    if (weekData.length > 0) localStorage.setItem(CACHE_KEY_RC, JSON.stringify(weekData));
  }, [weekData]);
  ...
  async function loadProcesses() {
    ...
    try {
      ...
      const rcProcesses = allProcesses.filter(p => 
          (!p.warehouse || p.warehouse === 'RC' || p.warehouse === 'All') &&
          (p.name.toLowerCase().includes('handover') || 
           p.name.toLowerCase().includes('sort') || 
           p.name.toLowerCase().includes('consol') || 
           p.name.toLowerCase().includes('over') || 
           p.name.toLowerCase().includes('box') || 
           p.name.toLowerCase().includes('label') ||
           p.name.toLowerCase().includes('shipping'))
      );
      
      rcProcesses.sort((a, b) => {
          const getRank = (name: string) => {
              const n = name.toLowerCase();
              if (n.includes('handover')) return 1;
              if (n.includes('sorter auto') || n.includes('automatico')) return 2;
  ...
  const handleTrainAI = async () => {
    ...
    if (!confirm(`Deseja enviar os dados de ${currentDayName} para o Cérebro do RC aprender?`)) return;
    ...
    const promises = processOrder.map(async (proc) => {
        ...
        if (samples.length > 0) {
            ...
            try {
                await api.post('/aimodelrc/train', {
                    processId: proc.id,
                    samples: samples
                });
                successCount++;
            } catch (e) {
                console.error(`Erro ao treinar proc ${proc.id}`, e);
            }
        }
    });
    ...
    if (totalSamplesFound === 0) {
        alert("⚠️ Nenhuma alocação de pessoas encontrada para treinar.");
    } else {
        alert(`✅ IA do RC Calibrada com Sucesso!\nProcessos: ${successCount}`);
    }
  };

  const handleSmartDistributeWeek = async () => {
    if (!confirm(`O Servidor RC vai calcular o fluxo (Handover -> Sorter -> Boxing). Continuar?`)) return;
    ...
    try {
        ...
        const response = await api.post('/simulation/smart-distribute-rc', payload);
  ...
  const handleSaveScenario = async () => {
    ...
    try {
        ...
        const response = await api.post('/simulation/rc', payload);
        alert(`✅ Cenário RC salvo! ID: ${response.data.id}`);
  ...
  const handleOpenLoadModal = async () => {
      try {
          const response = await api.get('/simulation/rc');
  ...
  const handleLoadScenario = async (id: number) => {
      ...
      try {
          const response = await api.get(`/simulation/rc/${id}`);
  ...
  const handleDeleteScenario = async (id: number) => {
      ...
      try {
          await api.delete(`/simulation/rc/${id}`);
  ...
  const handleImportWeeklyForecast = async () => {
    if (!startDate || !endDate) return alert("Selecione a semana.");
    setImporting(true);
    try {
        ...
        for (const ym of monthsToFetch) {
            ...
            try {
                const response = await api.get('/forecast/monthly', { params: { year, month } });
        ...
        setWeekData(prev => {
            ...
            datesInRange.forEach((dateStr, idx) => {
                if (idx < 7) { 
                    ...
                    if (dayForecast) {
                        newData[idx].volume = dayForecast.rcRecebimento * 1000; 
                    } else {
  ...
  const handleResetData = () => {
    if(!confirm(`Resetar todo o planejamento RC?`)) return;
    localStorage.removeItem(CACHE_KEY_RC);
    ...
    weekData.forEach((dayData, dayIdx) => {
       ...
       processOrder.forEach((proc, idx) => {
          ...
          hoursArray.forEach((hour, hIdx) => {
             ...
             if (is('handover')) {
                 const share = DOCK_ARRIVAL_MATRIX[dayIdx][hour];
                 input = dayData.volume * (share / 100);
             } 
    ...
    {isSaveModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm animate-fade-in-up">
            <div className="bg-white p-6 rounded-2xl shadow-2xl w-96">
                <h3 className="text-lg font-bold mb-4 flex items-center gap-2"><Save size={20} className="text-blue-600"/> Salvar Cenário RC</h3>
                <input 
                    type="text" 
                    placeholder="Nome do Cenário (ex: Black Friday RC)" 
                    className="w-full border border-gray-300 rounded-xl p-3 mb-4 outline-none focus:border-blue-600 font-bold text-gray-700"
                    value={scenarioName}
                ...
                <div className="flex gap-2 justify-end">
                    ...
                    <button onClick={handleSaveScenario} className="px-6 py-2 bg-blue-600 text-white font-bold rounded-lg hover:bg-blue-700">Salvar</button>
                </div>
            </div>
        </div>
    )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm animate-fade-in-up">
            <div className="bg-white p-6 rounded-2xl shadow-2xl w-[600px] max-h-[80vh] flex flex-col">
                <div className="flex justify-between items-center mb-4">
                    <h3 className="text-lg font-bold flex items-center gap-2"><FolderOpen size={20} className="text-blue-600"/> Carregar Simulação RC</h3>
        ...
        <div className="bg-white rounded-2xl p-6 relative z-10 flex flex-col xl:flex-row items-center justify-between gap-6">
            <div>
                <h1 className="text-4xl font-black text-gray-800 tracking-tight flex items-center gap-3">
                    WFM <span className="text-blue-600">Planner RC</span>
                </h1>
                <div className="flex items-center gap-2 mt-3 bg-blue-50 p-2 rounded-xl w-fit border border-blue-100">
                    <Truck size={16} className="text-blue-600" />
                    <span className="text-blue-600 font-bold">Visão Recebimento (Inbound)</span>
                </div>
            </div>
            ...
            <div className="flex flex-col md:flex-row items-center gap-4 bg-gray-50 p-3 rounded-2xl border border-gray-200">
                <div className="flex items-center gap-4">
                    <div className="flex flex-col">
                        <span className="text-[10px] font-bold text-gray-400 uppercase mb-1">Semana De</span>
                    ...
                    <div key={day} onClick={() => setActiveDayIndex(idx)} className={`flex-shrink-0 snap-start w-[180px] cursor-pointer p-4 rounded-2xl border transition-all duration-300 relative overflow-hidden group ${isSelected ? 'bg-white border-blue-600 ring-4 ring-blue-100 shadow-xl scale-105 z-10' : 'bg-white border-gray-200 hover:border-blue-200 hover:shadow-md'}`}>
                        ...
                        <div className="space-y-2">
                            <div className={`p-2 rounded-lg ${isSelected ? 'bg-gray-50' : 'bg-gray-50/50'}`}>
                                ...
                                <input type="number" className="w-full bg-transparent font-bold text-gray-800 outline-none border-b border-dashed border-gray-300 focus:border-blue-600 text-sm" value={weekData[idx].volume} onChange={(e) => updateDayData(idx, 'volume', Number(e.target.value))} />
                            </div>
                        </div>
        ...
        <div className="p-6 border-b border-gray-100 bg-gray-50/50 grid grid-cols-1 lg:grid-cols-12 gap-6 items-end">
            <div className="lg:col-span-2">
                ...
                <div className="relative">
                    <select className="w-full appearance-none bg-white border border-gray-200 text-gray-700 font-bold py-3 px-4 rounded-xl shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-100 focus:border-blue-600 transition-all cursor-pointer" value={currentDayData.shiftStart} onChange={e => updateCurrentDay('shiftStart', Number(e.target.value))}>
                ...
                return (
                    <div key={proc.id} className="group hover:bg-gray-50/30 transition-colors">
                        <div className="px-6 py-4 flex items-center justify-between">
                            <div className="flex items-center gap-4">
                                <div className={`w-10 h-10 rounded-xl flex items-center justify-center shadow-sm ${proc.type === 'Inbound' ? 'bg-blue-100 text-blue-600' : 'bg-indigo-100 text-indigo-600'}`}>
                                    ...
                                    <table className="min-w-full divide-y divide-gray-200">
                                        ...
                                        <tbody className="divide-y divide-gray-200 bg-white">
                                            ...
                                            <tr className="bg-gray-100/50">
                                                <td className="sticky left-0 z-20 bg-gray-100 py-2 pl-4 pr-3 text-xs font-bold text-gray-600 border-r border-gray-200"><div className="flex flex-col gap-1"><span className="flex items-center gap-1 text-emerald-600"><CheckCircle2 size={10}/> Produção</span><span className="flex items-center gap-1 text-red-500"><AlertTriangle size={10}/> Backlog</span></div></td>
                                                {data.map((d, i) => (
                                                    <td key={i} className="p-1 border-r border-gray-200 text-center align-middle">
                                                        <div className="flex flex-col gap-1">
                                                            ...
                                                            <span className={`text-[10px] font-bold px-1 rounded ${d.backlog > 5000 ? 'text-white bg-red-500' : d.backlog > 0 ? 'text-red-500 bg-red-50' : 'text-gray-300'}`}>{d.backlog > 0 ? Math.round(d.backlog) : '-'}</span>
                                                        </div>
                                                    </td>
                                                ))}
                                            </tr>
